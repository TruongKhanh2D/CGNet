...
class ViTCLIP(nn.Module):
    ...
    def get_text_embeddings(self, text_tokens, normalize=True):
        ...
        x = self.clip_model.token_embedding(text_tokens).to(self.dtype) # T
    ...
    def get_visual_feats_bchw(self, x):
        ...
        batch_size, num_channels, h_patches, w_patches = x.shape  # x shape = [B, C, H_patches, W_patches] = [2, 1024, 32, 32]
        ...
        # 添加分类 token
        x = torch.cat([vit_model.class_embedding.unsqueeze(0).expand(x.shape[0], 1, -1).to(x.dtype), x], dim=1) # [2, 1025, 1024]
        pos_embed = vit_model.positional_embedding.to(x.dtype) # [577, 1024]
